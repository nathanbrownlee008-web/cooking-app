...
// ===== BROWNIE TITLE + SECTION SEPARATOR FIX =====
(function () {
  if (window.__brownieTitleSectionFixDone) return;
  window.__brownieTitleSectionFixDone = true;

  function fixBrownie(recipe) {
    if (!recipe || !recipe.title) return;
    const name = String(recipe.title).toLowerCase();
    if (!name.includes("brownie")) return;

    recipe.title = "Brownies with Chocolate Sauce";
  ...
  collections.forEach(col => col.forEach(fixBrownie));

  if (typeof openRecipe === "function" && !window.__brownieSectionVisualFixDone) {
    ...
    openRecipe = function(recipeId) {
      ...
      ingredientNodes.forEach(node => {
        ...
        if (text === "BROWNIES" || text === "FUDGE SAUCE") {
          node.classList.add("ingredient-section-header");
        }
        if (text === "") {
          node.classList.add("ingredient-spacer");
          node.innerHTML = "&nbsp;";
        }
      });
    };
    window.openRecipe = openRecipe;
    window.__brownieSectionVisualFixDone = true;
  }
