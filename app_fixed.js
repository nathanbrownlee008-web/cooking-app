...
function parseTimeRangeLabel(text) {
  const str = String(text || "");
  const range = str.match(/(\d+)\s*(?:-|–|to)\s*(\d+)\s*(minutes?|mins?|hours?)/i);
  if (range) {
    const min = Number(range[1]);
    const max = Number(range[2]);
    const unit = /hour/i.test(range[3]) ? "hours" : "min";
    return {
      min: unit === "hours" ? min * 60 : min,
      max: unit === "hours" ? max * 60 : max,
      label: unit === "hours" ? `${range[1]}–${range[2]} hours` : `${min}–${max} min`
    };
  }

  const single = str.match(/(\d+)\s*(minutes?|mins?|hours?)/i);
  if (single) {
    const value = Number(single[1]);
    const unit = /hour/i.test(single[2]) ? "hours" : "min";
    return {
      min: unit === "hours" ? value * 60 : value,
      max: unit === "hours" ? value * 60 : value,
      label: unit === "hours" ? `${value} hour${value === 1 ? "" : "s"}` : `${value} min`
    };
  }
...
function inferStepHeat(title, body) {
  const lower = `${title} ${body}`.toLowerCase();
  if (/oven/.test(lower)) return "Oven";
  if (/fridge|chill/.test(lower)) return "Fridge";
  if (/heat 5|hob 5/.test(lower)) return "Hob 5";
  if (/heat 4|hob 4/.test(lower)) return "Hob 4";
  if (/heat 3|hob 3/.test(lower)) return "Hob 3";
  if (/heat 1|heat 2|hob 1|hob 2/.test(lower)) return "Hob 1–2 low";
  if (/rest|room temperature|dry the meat|pat dry|season|prep/.test(lower)) return "No heat";
  return "";
}

function inferStepTime(title, body) {
  const lower = `${title} ${body}`.toLowerCase();
  const explicit = parseTimeRangeLabel(lower);
  if (explicit) return explicit.label;
  if (/room temperature|take out.*20 ?(?:to|-) ?30/.test(lower)) return "20–30 min";
  if (/preheat/.test(lower)) return "2–3 min";
  if (/sear first side/.test(lower)) return "3–4 min";
  if (/flip|second side|baste/.test(lower)) return "3–4 min";
  if (/optional finish|cover/.test(lower)) return "2–3 min";
  if (/check temperature/.test(lower)) return "1 min";
  if (/rest/.test(lower)) return "5–10 min";
  return "";
}
...
function sumStepTimes(steps) {
  ...
  steps.forEach(step => {
    const parsed = parseTimeRangeLabel(step.time);
    if (!parsed) return;
    totalMin += parsed.min;
    totalMax += parsed.max;
    found = true;
  ...
  if (!found) return "See steps";
  return totalMin === totalMax ? `${totalMin} min` : `${totalMin}–${totalMax} min`;
}

function splitMethodIntoStructuredSteps(methodLines) {
  ...
  const isLikelyTitle = (line) => {
    ...
    if (/^(heat|cook|add|remove|rest|cover|baste|take|pat|season|insert|check|place|pour|mix|stir|whisk|fold|preheat)/.test(lower)) return false;
    return true;
  };
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    steps.push({
      title,
      heat: inferStepHeat(title, body),
      time: inferStepTime(title, body),
      body
    });
  }
...
function buildMergedSteps(title, ingredientSections, methodLines, category) {
  ...
  return splitMethodIntoStructuredSteps(methodLines);
}
...
function parseRecipeFromText(raw) {
  const cleanedRaw = normaliseRawText(raw);
  const category = inferCategory(cleanedRaw);
  const { title, ingredientSections, methodLines } = splitIntoSections(cleanedRaw);
  ...
  const totalTime = category === "Desserts" && /brownie/i.test(title) ? "35–45 min" : sumStepTimes(steps);
